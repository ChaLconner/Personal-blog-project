...
adminRouter.get('/posts', protectAdmin, async (req, res) => {
  try {
    ...
    const { data: posts, error } = await supabase
      ...
      .select(`
        ...
        category_id,
        status_id,
        categories!fk_posts_categories(id, name),
        statuses!fk_posts_statuses(id, status)
      `)
    ...
    const transformedPosts = (posts || []).map(post => ({
      ...
      status: post.statuses?.status || (post.status_id ? (post.status_id === 1 ? 'draft' : 'published') : null),
...
// Create new post
adminRouter.post('/posts', protectAdmin, async (req, res) => {
  try {
...
// Update post
adminRouter.put('/posts/:id', protectAdmin, async (req, res) => {
  try {
...
adminRouter.get('/posts/:id', protectAdmin, async (req, res) => {
  try {
    const postId = parseInt(req.params.id);
...
// Delete post
adminRouter.delete('/posts/:id', protectAdmin, async (req, res) => {
  try {
    const postId = parseInt(req.params.id);
...
adminRouter.get('/comments', protectAdmin, async (req, res) => {
  try {
...
adminRouter.delete('/comments/:id', protectAdmin, async (req, res) => {
  try {
...
adminRouter.get('/categories', protectAdmin, async (req, res) => {
  try {
...
// Create new category
adminRouter.post('/categories', protectAdmin, async (req, res) => {
  try {
    const { name } = req.body;
    ...
    if (checkError) {
      ...
      return res.status(500).json({ error: "Error checking existing categories", details: checkError.message });
    }
...
// Update category
adminRouter.put('/categories/:id', protectAdmin, async (req, res) => {
  try {
    ...
    if (checkError) {
      console.error('Error checking existing categories on update:', checkError);
      return res.status(500).json({ error: "Error checking existing categories", details: checkError.message });
    }
...
// Delete category
adminRouter.delete('/categories/:id', protectAdmin, async (req, res) => {
  try {
    const categoryId = parseInt(req.params.id);
